...
function kpiHTML(){
  ...
  return '<div class="homekpi">'
    ...
    + tile('🏆 최다 출원인', top
        ...
        + '가장 많은 곳입니다. 유럽특허청(EPO)이 각국에서 모아 수록한 전 세계 서지 데이터를 조회하며, '
        + 'IP5 나 국내(KR) 로 한정하지 않습니다. 다만 관청마다 수록 범위·시점이 달라 최근 공개분에는 '
    ...
    + '<p class="kpinote">특허 수치는 <b>전 세계 공개 데이터</b>(유럽특허청 수록 기준)입니다. 최근 '
    + lookback+'일 공개분을 전력 CPC 로 조회한 값입니다. 같은 발명이 여러 나라에 공개되면 각각 세므로 '

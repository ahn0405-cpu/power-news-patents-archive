...
def _origin_checks() -> None:
    ...
    # 것은 **국적 미상을 조용히 빼는 것**이다 — 실측 10,582건 중 6,612건이
    # 미상이고 전부 해외 공보다(중국 공보가 국적 칸을 비워 보낸다). 빼고 그리면
    # 'KR 압도적' 이라는, 우리가 확인한 범위의 모양일 뿐인 그림이 나온다.
    print("\n· 출원인국 → 기술 → 공개 특허청")
    check("sankeyHTML()" in js and "snkFlows" in js, "Sankey 를 그린다")
    check("getItem('pnp_snkMode')" in js and "setItem('pnp_snkMode'" in js,
          "고른 보기를 쓰고 또 읽는다")
    check("'[data-snk]'" in js and 'data-snk="all"' in js
          and 'data-snk="known"' in js,
          "두 보기 버튼과 그 처리기가 함께 있다")
    ks3 = js.find("const SNK =")
    ke3 = js.find("// ── 분야별 경쟁 구도")
    check(0 <= ks3 < ke3, "Sankey 블록을 떼어낼 수 있다")
    if 0 <= ks3 < ke3:
        # 한국 4건(국내 3 · 미국 1) · 중국 2건(자국) · 미상 4건(전부 중국 공보)
        mk = lambda co, cat, off, n: [
            {"aCountry": co, "category": cat, "office": off} for _ in range(n)]
        sit = (mk("KR", "y04s10", "KR", 3) + mk("KR", "y04s10", "US", 1)
               + mk("CN", "y04s10", "CN", 2) + mk("", "y04s10", "CN", 4)
               + mk("KR", "zzz", "KR", 5)          # 없는 분야 → 안 들어온다
               + mk("KR", "y04s10", "ZZ", 5))      # 없는 특허청 → 안 들어온다
        sfeed = {"patents": {
            "categories": [{"key": "y04s10", "emoji": "⚡", "name": "발전"}],
            "offices": [{"code": "KR", "emoji": "🇰🇷", "name": "한국"},
                        {"code": "CN", "emoji": "🇨🇳", "name": "중국"},
                        {"code": "US", "emoji": "🇺🇸", "name": "미국"}],
            "countries": [{"code": "KR", "emoji": "🇰🇷", "name": "한국"},
                          {"code": "CN", "emoji": "🇨🇳", "name": "중국"}],
            "items": sit}}
        prog4 = ("const localStorage={getItem(){return null;},setItem(){}};\n"
                 + js[ks3:ke3]
                 + "\nfunction esc(s){return String(s).replace(/[&<>\"]/g,"
                   "c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));}\n"
                 + f"const FEED={json.dumps(sfeed, ensure_ascii=False)};\n"
                   "const boot=snkMode;\n"
                   "SNK_MIN_TOTAL=1;\n"     # 열 건짜리 표본으로도 그리게

                   "const fa=snkFlows('all'), fk=snkFlows('known');\n"
                   "const html=sankeyHTML();\n"
                   "const nodeH=[...html.matchAll(/class=\"snkn\"[^>]*height=\"([\\d.]+)\"/g)]"
                   ".map(m=>+m[1]);\n"
                   "const ribH=[...html.matchAll(/class=\"snkl\"[^>]*v([\\d.]+)C/g)]"
                   ".map(m=>+m[1]);\n"
                   "console.log(JSON.stringify({boot, nodeH, ribH,"
                   "allN:Object.keys(fa.flows).reduce((s,k)=>s+fa.flows[k],0), naA:fa.na,"
                   "knownN:Object.keys(fk.flows).reduce((s,k)=>s+fk.flows[k],0), naK:fk.na,"
                   "nodesAll:snkNodes(fa.flows,0).map(n=>n.key+':'+n.v),"
                   "nodesKnown:snkNodes(fk.flows,0).map(n=>n.key+':'+n.v)}));")
        prog4 = prog4.replace("\\\\", "\\")
        with tempfile.NamedTemporaryFile("w", suffix=".js", delete=False,
                                         encoding="utf-8") as f:
            f.write(prog4)
            path4 = f.name
        try:
            out4 = subprocess.run(["node", path4], capture_output=True, text=True,
                                  timeout=30)
            ok4 = out4.returncode == 0
            check(ok4, "Sankey JS 가 실행된다"
                       + ("" if ok4 else f" ({out4.stderr.strip()[:200]})"))
            r4 = json.loads(out4.stdout) if ok4 else None
        finally:
            try:
                __import__("os").unlink(path4)
            except OSError:
                pass
        if r4:
            check(r4["boot"] == "all",
                  f"기본은 '전체' 다 — 확인분만 먼저 보이면 그게 전체인 줄 읽힌다 "
                  f"(받은 값 {r4['boot']!r})")
            check(r4["allN"] == 10 and r4["knownN"] == 6,
                  f"전체는 미상까지 세고 확인분은 뺀다 (받은 값 {r4['allN']}·{r4['knownN']})")
            check(r4["naA"] == 4 and r4["naK"] == 4,
                  f"좁혀 보는 중에도 **뺀 건수를 계속 센다** — 화면이 몇 건을 "
                  f"빼고 그렸는지 밝혀야 한다 (받은 값 {r4['naA']}·{r4['naK']})")
            check(":" in "".join(r4["nodesAll"]) and r4["nodesAll"][-1].startswith(":"),
                  f"미상은 맨 아래에 둔다 — 위에 두면 그림의 첫인상을 '모름' 이 "
                  f"차지한다 (받은 값 {r4['nodesAll']})")
            check(not any(n.startswith(":") for n in r4["nodesKnown"]),
                  "확인분 보기에는 미상 마디가 아예 없다")
            # 분야·특허청이 목록에 없는 건은 애초에 안 들어온다(엉뚱한 마디가 생긴다)
            check(r4["allN"] == 10,
                  "모르는 분야·특허청 코드는 흐름에 넣지 않는다")
            # 굵기가 건수에 **비례**해야 열끼리 견줄 수 있다. 열마다 배율을
            # 따로 잡으면 같은 100건이 왼쪽에선 굵고 오른쪽에선 얇아진다.
            # 첫 열: 한국 4건 · 중국 2건 · 미상 4건 → 높이가 2:1:2 여야 한다.
            nh = r4["nodeH"][:3]
            ratio_ok = (len(nh) == 3 and nh[1] > 0
                        and abs(nh[0] / nh[1] - 2) < 0.02
                        and abs(nh[2] / nh[1] - 2) < 0.02)
            check(ratio_ok, f"마디 굵기가 건수에 비례한다 (4:2:4 → 받은 값 {nh})")
            # 같은 흐름의 왼쪽 띠와 오른쪽 띠는 높이가 같아야 한다(열마다 배율을
            # 따로 잡으면 여기서 갈라진다). 띠 높이 목록이 좌우로 같은 다중집합.
            rb = [round(x, 2) for x in r4["ribH"]]
            half = len(rb) // 2
            check(len(rb) >= 6 and len(rb) % 2 == 0
                  and sorted(rb[:half]) == sorted(rb[half:]),
                  f"같은 흐름의 좌우 띠 굵기가 같다 (받은 값 {rb})")
    # 세로 배율은 세 열이 같아야 한다. 열마다 따로 잡으면 같은 100건이 왼쪽에선
    # 굵고 오른쪽에선 얇아져, 굵기를 열끼리 견줄 수 없다.
    check("const scale=(inner-maxGaps*gap)/total;" in js,
          "세 열이 같은 세로 배율을 쓴다 (굵기를 열끼리 견줄 수 있게)")

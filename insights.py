...
산출물(dict):
  ...
  cross      : 뉴스↔특허 교차 신호 [{key,name,emoji,news,patent,both}]
...
def _cross(news_days: dict, patent_weeks: dict, recent_from, week_from):
    """뉴스(최근 N일)·특허(최근 M주)에서 카테고리별 활동량 → 둘 다 활발한 주제 강조."""
    ncnt = {}
    for d, a in _iter_articles(news_days):
        if d is not None and d >= recent_from:
            k = a.get("category", "etc")
            ncnt[k] = ncnt.get(k, 0) + 1
    pcnt = {}
    for wk in patent_weeks:
        wd = _to_date(wk)
        if wd is None or wd < week_from:
            continue
        for p in patent_weeks[wk].get("patents", []):
            k = p.get("category", "etc")
            pcnt[k] = pcnt.get(k, 0) + 1
    rows = []
    for c in ncfg.CATEGORIES:      # 뉴스 카테고리 기준(교집합 라벨은 뉴스 쪽 사용)
        k = c["key"]
        n, pt = ncnt.get(k, 0), pcnt.get(k, 0)
        if n == 0 and pt == 0:
            continue
        rows.append({"key": k, "name": c["name"], "emoji": c["emoji"],
                     "news": n, "patent": pt, "both": n > 0 and pt > 0})
    # 둘 다 활발한 주제 먼저, 그다음 총량 큰 순
    rows.sort(key=lambda x: (x["both"], x["news"] + x["patent"]), reverse=True)
    return rows
...
def build(news_days: dict, patent_weeks: dict) -> dict:
    ...
    if not dates:
        return {"asOf": "", "trending": [], "catTrend": [], "cross": [],
    ...
    week_from = latest - timedelta(weeks=RECENT_WEEKS)
    return {
        ...
        "cross": _cross(news_days, patent_weeks, recent_from, week_from),
    }
